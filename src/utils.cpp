...
Status mat2Tensor(const cv::Mat& mat, Tensor& inputTensor) {
	//auto start = std::chrono::system_clock::now();
    int width = mat.cols;
    int height = mat.rows;
    int depth = mat.channels();
    cout<<"shape:"<<width<<","<<height<<","<<depth<<endl;
    const float *source_data = (float*)mat.data;
    auto inputImageMapped = inputTensor.tensor<float, 4>(); // 4 tensors
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (int x = 0; x < width; ++x) {
            ...
            for (int c = 0; c < depth; ++c) {
                ...
                inputImageMapped(0, y, x, c) = *source_value;
            }
        }
    }
